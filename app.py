    ...
    if analyze_btn and uploaded_file and job_description:
        with st.spinner(
            ...
            try:
                ...
                keyword_result = keyword_analyzer.calculate_match(
                    cv_keywords, jd_keywords
                )

                # Semantic analysis
                semantic_score = semantic_analyzer.compute_similarity(
                ...
                jd_skills = list(
                    keyword_analyzer.extract_keywords(job_description)["skills"]
                )
                ...
                gaps = gap_analyzer.analyze(
                    parsed_resume, jd_skill_names, cv_skill_names
                )
                ...
                with score_cols[0]:
                    render_score_card(
                        score_result["overall_score"], "Overall ATS Score"
                    )
                    ...
                    st.markdown(
                        f"### {grade_colors.get(grade, '⚪')} Grade: **{grade}** - {label}"
                    )
                ...
                with tab1:
                    ...
                    with col_b:
                        ...
                        if skill_coverage.get("missing_skills"):
                            render_skill_tags(
                                skill_coverage["missing_skills"], "missing"
                            )
                        else:
                    ...
                    if skill_coverage.get("taxonomy_matches"):
                        ...
                        for cv_skill, jd_skill in skill_coverage["taxonomy_matches"][
                            :5
                        ]:
                            st.markdown(
                ...
                with tab2:
                    ...
                    if gaps["high_priority"]:
                        ...
                        for gap in gaps["high_priority"]:
                            with st.expander(
                                f"**{gap.type.title()}**: {gap.description}"
                            ):
                ...
                with tab3:
                    ...
                    if bullet_suggestions:
                        for i, suggestion in enumerate(bullet_suggestions[:5]):
                            with st.expander(
                                f"Bullet {i + 1}: {suggestion['original'][:60]}..."
                            ):
                ...
                with tab4:
                    ...
                    for item in checklist:
                        ...
                        st.checkbox(
                            f"{icon} {item['item']}", key=f"check_{item['item'][:20]}"
                        )
                ...
                with tab5:
                    ...
                    recommendations.extend(
                        [g.suggestion for g in gaps["high_priority"][:3]]
                    )
                ...
                action_cols = st.columns(
                    min(len(priority_actions), 3) if priority_actions else 1
                )
                ...
                if priority_actions:
                    ...
                    st.success(
                        "Your resume is well-optimized! No major actions needed."
                    )
